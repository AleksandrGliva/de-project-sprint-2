...
SELECT 
  DISTINCT vad[1] :: INT AS agreement_id, 
  vad[2] :: VARCHAR(12) AS agreement_number, 
...
SELECT 
  DISTINCT std[1] :: VARCHAR(2) AS transfer_type, 
  std[2] :: VARCHAR(10) AS transfer_model, 
...
FROM 
  ...
  JOIN public.shipping_transfer AS st ON shipping_all.std[2] = st.transfer_model 
  AND shipping_all.std[1] = st.transfer_type 
...
INSERT INTO public.shipping_status (
  shipping_id, status, state, shipping_start_fact_datetime, 

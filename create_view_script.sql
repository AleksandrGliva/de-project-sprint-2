CREATE 
OR REPLACE VIEW shipping_datamart AS (
  SELECT 
    ss.shipping_id, 
    vendor_id, 
    transfer_type, 
    ...
    CASE WHEN status = 'finished' THEN 1 ELSE 0 END :: bool AS is_shipping_finish, 
    CASE WHEN shipping_end_fact_datetime > shipping_plan_datetime THEN EXTRACT(
      DAY 
      FROM 
        (
    ...
    payment_amount, 
    -- в задании есть, а на схеме данного столбца нет
    payment_amount * (
      shipping_country_base_rate + agreement_rate + shipping_transfer_rate
    ) as vat, 
    payment_amount * agreement_commission AS profit 
  FROM 
    public.shipping_status AS ss 
    JOIN public.shipping_info AS si ON ss.shipping_id = si.shipping_id 
